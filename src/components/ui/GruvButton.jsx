...
import { Loader2 } from 'lucide-react'; // Example icon if needed, though user button doesn't use it directly

// Colors for the liquid animation
const COLORS = {
    color1: '#FFFFFF',
    color2: '#1E10C5',
    color3: '#9089E2',
    ...
    return (
        <div className="flex justify-center my-4">
            <a
                ...
                className="relative inline-block w-48 h-12 mx-auto group bg-black border-white border rounded-lg hover:scale-105 transition-transform duration-300"
                onMouseEnter={() => setIsHovered(true)}
                onMouseLeave={() => setIsHovered(false)}
            >
                <div className="absolute w-[112.81%] h-[128.57%] top-[8.57%] left-1/2 -translate-x-1/2 filter blur-[10px] opacity-70 pointer-events-none">
                    <span className="absolute inset-0 rounded-lg bg-[#d9d9d9] filter blur-[3.5px]"></span>
                ...
                <div className="relative w-full h-full overflow-hidden rounded-lg bg-black">
                    ...
                    {/* Overlay layers for glass effect */}
                    {[1, 2, 3].map((i) => (
                        <span
                            key={i}
                            className={`absolute inset-0 rounded-lg border-solid border-[2px] border-white/10 mix-blend-overlay filter blur-[1px]`}
                        ></span>
                    ))}
                    ...
                    >
                        <span className="text-white text-sm font-bold tracking-widest group-hover:text-white transition-colors uppercase">
                            GRÜV. LABEL
                        </span>
                    </div>
                </div>
